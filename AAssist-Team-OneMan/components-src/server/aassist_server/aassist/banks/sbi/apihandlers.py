...
def LoginUser(user,passwd):
    ...
    if apasswd[0] == passwd:
        safe_token = parapool.assignnew(gen_token,(user+str(time.time()),)).get()
...
def FetchAccounts(user,token):
    ...
    udata = parapool.assignnew(mysqlc_p.handle.fetchone,()).get()
    if udata != None and udata[0] == token:
        parapool.assignnew(mysqlc_p.handle.execute,("SELECT accno FROM accounts WHERE cifno = %s",(udata[1]),)).get()
...
def SBICredit(srcacc,srcbank,accno,amount):
    mysqlc_p = MySQLdb(api_bearer["DB_NAME"])
    mysqlc = mysqlc_p.mysqlc
    succ_code = False
    try:
...
def SBIDebit(accno,amount,credacc,credbank):
    mysqlc_p = MySQLdb(api_bearer["DB_NAME"])
    mysqlc = mysqlc_p.mysqlc
    succ_code = False
    try:
    ...
    if udata != None and udata[0] == token:
        ...
        if dest == "INTRA_BANK":
            ...
            if int(balance[0]) >= int(amount):
                if SBIDebit(accno, amount, benefacc,"sbi"):
            ...
            if int(balance[0]) >= int(amount):
                if bcode == "axis":
                    from aassist.banks.axis.apihandlers import AXISCredit
                    response_code = [-1]
                    if SBIDebit(accno, amount, benefacc,bcode):
                        if AXISCredit(accno,"sbi",benefacc,amount):
                            parapool.assignnew(mysqlc_p.handle.execute, ("SELECT balance FROM accounts WHERE accno = %s AND cifno = %s",(accno,udata[1]),)).get()
                            curr_bal = parapool.assignnew(mysqlc_p.handle.fetchone,()).get()
                            parapool.assignnew(mysqlc.close, ())
                            response_code = [curr_bal[0]]
                    return response_code
                else :
                    return [-1]
